...
Global conditioning also helps CRFs avoid the label bias problem that afflicts MEMMs. Label bias comes from local probability normalization [@Hannun2019]. CRFs, unlike MEMMs, employ global normalization — the normalization factor $Z$ is a function of the entire input sequence $x$ — so they are able to escape the label bias problem.
