...
Many of the most informative features – those with the largest absolute weights – encode the identities of words that appear frequently in the train set:

<MEMMMostInformativeFeatures />

Other features are more revealing. For example, if the current word has shape “X.X”, then the current state is unlikely to be “O” (the corresponding feature-state pair has large negative weight), meaning the word is likely part of a named entity. With only one feature-state pair, the model has learned about two-letter initialisms, like “U.S.“ and “U.N.“. There are plenty examples of how it has found and encoded many other linguistic patterns using the same mechanism.

Here’s a live version of the trained model with Viterbi decoding:
...
MEMMs’ discriminative structure confers great benefits, namely the ability to model word features, but there’s a downside: it makes MEMMs susceptible to the label bias problem. First recorded by Bottou[@Bottou1991], this problem mostly affects discriminative models, causing certain states to ignore their observations.

MEMMs are susceptible to label bias because they are locally normalized. Normalization is necessary to make probability distributions to sum up to one. Here, the “local” in local normalization means only transition distributions between neighboring states need to be normalized, not the “global”, sequence-wide joint distributions. Hannun[@Hannun2019] provides an excellent explanation of how local normalization can cause certain states to ignore/take little notice of their observations.

While the label bias problem doesn’t render models useless, it still has a notable effect on predictions, causing demonstrably higher error rates.[@Lafferty2001]

We can consider getting rid of local normalization to avoid the problem. That leads us to Conditional Random Fields – a class of globally-normalized, undirected probabilistic models.
