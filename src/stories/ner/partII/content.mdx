...
\scriptsize \textrm{for every $i \in S = \{2,…, N\}$}
$$
...
\scriptsize \textrm{for every $i \in S = \{2,…, N-1\}$}
$$
...
\scriptsize \textrm{for every $i \in S = \{1, 2,…, N\}$}
$$
...
The conditional probability $p(y_i | x_i)$ is also assumed to be time-homogenous, further reducing the complexity of the model.
