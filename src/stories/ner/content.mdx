...
Given the massive number of potential paths, brute force computations are not an option. Luckily, there is an efficient dynamic algorithm that accomplishes the same goal with substantially less computational complexity: the Viterbi algorithm [@Viterbi1967]. It constructs a trellis-shaped memory structure to progressively find the most likely path as it progesses through the given sequence.
