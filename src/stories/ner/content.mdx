...
CRFs are a notable class of Markov Random Fields – probability distributions defined by _undirected_ graphs where each node is a random variable:
...
In Markov Random Fields, the relationship between each variable pair is modeled with a scoring function, called a _factor_, that assigns higher scores if the variables’ values are in agreement, for example:
