...
where $s\prime$ is the previous state, $s$ is the current state, $o$ is the current observation, $a$ is a feature-state pair, $\lambda_a$ is the weight for $a$, and $Z(o, s\prime)$ is a normalizing term to make the distribution sum to one across all next states $s$. [@McCallum2000]
