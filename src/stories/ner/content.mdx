...
Maximum Entropy Markov Models (MEMMs) resolve some of the concerns with HMMs by way of a slight yet meaningful difference in structure:
...
Suppose we have a simple system with two variables, $X$ and $Y$. In many cases, we want to make predictions on one variable using observations from the other. A generative model does that by first getting a good understanding of the generative process – learning the joint distribution $p(X, Y)$ and then using Bayes' rule to get the probability distribution over the unobserved variable. On the other hand, a discriminative model would directly compute the probability of the unobserved variable conditioned on the observations [@Ng2001].

While HMMs are generative models, MEMMs are discriminative.
