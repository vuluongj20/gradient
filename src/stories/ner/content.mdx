...
Luckily, there is an efficient dynamic algorithm that returns the most likely path with relatively low computational complexity: the Viterbi algorithm [@Viterbi1967]. It constructs a trellis-shaped memory structure to progressively find and store the most likely path as it progesses through the given sequence. For more details, refer to the explanation in the book _Speech and Language Processing_ by Jurafsky & Martin [@Jurafsky2021].

### Results

<LivePrediction
	models={['hmm']}
	label="Most likely state sequence estimation by a trained HMM:"
/>
